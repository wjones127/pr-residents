...
func laneRow(r *prr.Record) RowView {
	return RowView{
		Repo:      r.Repo,
		Number:    r.Number,
		URL:       r.URL,
...
// merge; housekeeping: batched in input order).
func BuildView(records []*prr.Record, dateLabel string) RoundsView {
	...
	for _, r := range fresh {
		view.Fresh = append(view.Fresh, laneRow(r))
	}
	for _, r := range rereview {
		view.Rereview = append(view.Rereview, laneRow(r))
	}
	for _, r := range house {
